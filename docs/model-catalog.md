...
10. **deepseek-v4-flash-vision-exp 接入（2026-08-21）**：DeepSeek 新上线的实验性视觉模型，ID 带 `-exp` 后缀；官方规格 1M 上下文 / 384K 输出，图片输入（JPEG/PNG/GIF/WebP，三种传图方式，单图 ≤384 token，仅限 user 消息），**价格与 deepseek-v4-flash 完全相同**；功能表明确列出 Responses API 支持（官方 responses 文档页同日确认上线，见关键发现 1 更新）。条目沿用同系 `openai-responses` + `reasoning.effort: max`（档位 low/high/xhigh/max）配置并补 `vision: true`；**提交信息生成改由 `libiaoCopilot.commitModel` 独立配置项控制**（默认 `deepseek-v4-flash`），不再使用 `useForCommitGeneration` 字段。**待网关实测**：`/v1/responses` 连通性、effort 回显、视觉输入是否经网关转发正常（new-api 对 Responses 视觉块的翻译待验证）。
