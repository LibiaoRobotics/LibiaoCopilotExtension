...
9. **实参锚定与脱离实际防空谈**：凡涉及机制分析、性能评估、架构合理性或重构建议，必须先核验当前工程的真实生效配置与数值边界（如模型实际配置的 `context_length`、`context_sizes`、真实触发水位），严禁脱离现场配置凭空推导；必须严格区分“常态高频路径”与“低频极端兜底”，明确给出触发门槛与概率，严禁将实际几乎不触发的兜底逻辑夸大为日常系统痛点。
