...
- **实时生成速度（TPS）显示**：请求流式输出期间，状态栏实时显示生成速度（每秒 token 数，含思考 token），停顿时显示「停滞」提示，请求结束恢复原状。实时值基于「字符数 × 校准系数」估算（按模型自动校准），结束后输出精确结算值（服务端 `completion_tokens` ÷ 流式时长）。可通过 `libiaoCopilot.tpsStatusBar`（默认 `true`）关闭。
- 新增一个隐藏测试功能（默认关闭，不随配置保存/导出/导入迁移）。
